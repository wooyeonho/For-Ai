...
export default async function WikiDocumentPage({
  ...
  const returnUrl = `/${locale}/wiki/${document.slug}`;
  const actionQuery = `lang=${encodeURIComponent(locale)}&return=${encodeURIComponent(returnUrl)}`;
    ...
    <article>
      ...
      <nav className="registry-panel" aria-labelledby="machine-links">
        ...
        <ul className="link-list">
          ...
          <li><Link href={`/report/${document.slug}?${actionQuery}`}>{t.wiki.correctionReport}</Link></li>
          <li><Link href={`/hallucination/${document.slug}?${actionQuery}`}>{t.wiki.hallucinationReport}</Link></li>
