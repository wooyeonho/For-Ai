      ...
      <main style={WRAP}>
        <header style={{ ...PANEL, marginBottom: 18 }}>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            <input aria-label="Admin password" type="password" value={secret} onChange={(event) => setSecret(event.target.value)} placeholder="Admin password" style={{ flex: "1 1 260px", padding: "10px 12px", border: "1px solid #d1d5db", borderRadius: 10 }} />
        ...
        <section id="recent-admin-actions" style={{ ...PANEL, marginTop: 18 }}>
          ...
          {(data?.dashboard?.recent_admin_actions?.length ?? 0) === 0 ? (
            <p style={{ color: "#6b7280" }}>관리자 cookie 인증 후 집계를 불러오면 최근 작업 로그가 표시됩니다.</p>
          ) : (
