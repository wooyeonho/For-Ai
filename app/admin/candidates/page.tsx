...
export default function CandidatesPage(){
  ...
  const load=useCallback(async()=>{
    setLoading(true);
    const params = new URLSearchParams({ status: filter, source_hint: sourceHintFilter });
    ...
    <div style={{minHeight:"100vh",background:"#f9fafb",padding:"24px",fontFamily:"sans-serif"}}>
      <div style={{maxWidth:860,margin:"0 auto"}}>
        ...
        <div style={{display:"flex",gap:8,marginBottom:16,flexWrap:"wrap",alignItems:"center"}}>
          <span style={{fontSize:12,fontWeight:600,color:"#6b7280"}}>출처 힌트</span>
          {[{key:"all",label:"전체"},{key:"with",label:"source hint 있음"},{key:"without",label:"source hint 없음"}].map(f=>(
            <button key={f.key} onClick={()=>setSourceHintFilter(f.key)} style={{padding:"5px 12px",borderRadius:20,fontSize:12,fontWeight:600,cursor:"pointer",border:sourceHintFilter===f.key?"none":"1px solid #d1d5db",background:sourceHintFilter===f.key?"#2563eb":"#fff",color:sourceHintFilter===f.key?"#fff":"#374151"}}>{f.label}</button>
          ))}
        </div>
            ...
            <div style={{display:"flex",justifyContent:"space-between",gap:12}}>
              <div style={{flex:1,minWidth:0}}>
                <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:6}}>
                  ...
                  {(!c.source_hints||c.source_hints.length===0)&&<span style={{fontSize:11,padding:"2px 8px",borderRadius:12,fontWeight:700,background:"#fef3c7",color:"#92400e"}}>⚠ source hint 없음</span>}
                </div>
            ...
            {selected===c.id&&(
              <div style={{marginTop:16,paddingTop:16,borderTop:"1px solid #f3f4f6"}} onClick={e=>e.stopPropagation()}>
                ...
                {c.source_hints?.length>0?<div style={{marginBottom:16}}>
                  ...
                  :<div style={{marginBottom:16,padding:"10px 12px",background:"#fffbeb",border:"1px solid #fde68a",borderRadius:8,fontSize:12,color:"#92400e",fontWeight:600}}>⚠ source_hints가 없습니다. 공식/platform/law/document 출처 확인 후 승인하세요.</div>}
