...
export default function CandidatesPage(){
  ...
  async function act(id:string,st:string){
    ...
    else flash(`❌ ${d.error??"권한 오류 — 관리자 세션 확인"}`,false);
  }
      ...
      <div style={{maxWidth:860,margin:"0 auto"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:24}}>
          <div><h1 style={{fontSize:22,fontWeight:700,margin:0}}>📋 후보 검토 큐</h1>
            <p style={{color:"#6b7280",fontSize:13,margin:"4px 0 0"}}>public submit → triage → AI claim generation → 🚀 공개 등록</p></div>
          <div style={{display:"flex",gap:8,alignItems:"center"}}>
            <AdminSecretField adminSecret={adminSecret} setAdminSecret={setAdminSecret} resetAdminSecret={resetAdminSecret} login={login} status={status} message={loginMessage} label="관리자 인증키" placeholder="ADMIN_SECRET" inputStyle={{ border: "1px solid #d1d5db", borderRadius: 6, padding: "6px 10px", fontSize: 13, width: 160 }}/>
