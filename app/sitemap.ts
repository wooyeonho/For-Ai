...
import { buildDocumentLanguageAlternates } from "../lib/seo";
...
function mergeDocumentEntries(entries: DocumentSitemapEntry[]): DocumentSitemapEntry[] {
  ...
  for (const entry of entries) {
    ...
    bySlug.set(entry.slug, {
      ...
      lastModified: entry.lastModified > existing.lastModified ? entry.lastModified : existing.lastModified,
    });
  }
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const documentPages = documentEntries.flatMap((document) => {
    const languages = buildDocumentLanguageAlternates(document.slug);
    const priority = document.canCite ? 0.9 : document.sourceCount > 0 ? 0.7 : 0.5;

    return SUPPORTED_LOCALES.map((locale) => ({
      url: documentPageUrl(document.slug, locale),
      lastModified: document.lastModified,
      changeFrequency: "weekly" as const,
      priority: locale === DEFAULT_LOCALE ? priority : Math.max(priority - 0.1, 0.4),
      alternates: {
        languages,
      },
    }));
  ...
  // Bible v7 Book IV section 9.4: check and changelog are discovery surfaces and
  // belong in the sitemap; feed.xml/changelog.xml (RSS) and /embed, /api/badge
  // are excluded (RSS feeds are advertised via alternates.types instead, and
  // embed/badge are non-canonical presentational surfaces).
  const checkAndChangelogPages: MetadataRoute.Sitemap = SUPPORTED_LOCALES.flatMap((locale) => [
    {
