...
function browserOriginSentAdminSecret(request: Request): boolean {
  return Boolean(request.headers.get("x-admin-secret") && (request.headers.get("origin") || request.headers.get("sec-fetch-site")));
}

function sessionCookieContext(request: Request): AdminAuthContext | null {
...
export async function authorized(request: Request): Promise<boolean> {
  if (browserOriginSentAdminSecret(request)) return false;
  return (await supabaseAuthContext(request)) !== null || sessionCookieContext(request) !== null || fallbackSecretContext(request) !== null;
}
...
export async function requireAdmin(request: Request, action: string): Promise<NextResponse | null> {
  ...
  if (browserOriginSentAdminSecret(request)) {
  ...
  const context = await supabaseAuthContext(request) ?? sessionCookieContext(request) ?? fallbackSecretContext(request);
  if (!context) {
