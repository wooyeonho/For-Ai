...
import { logAdminAuditEvent, missingSupabaseAdminEnv } from "@/lib/admin-api";
...
export async function generateCandidatesService(sb: SupabaseClient | null, request: Request, body: Record<string, unknown>): Promise<Result<Record<string, unknown>>> {
  const topic = String(body.topic ?? body.category ?? "").trim();
  const count = Math.min(Math.max(parseInt(String(body.count ?? "10"), 10), 1), 50);
  ...
  if (!topic) {
    return err("topic 필드가 필요합니다", 400);
  }

  const available = getAvailableProviders();
  if (available.length === 0) {
    return err("No AI providers configured. Set at least one API key.", 500);
  }
  ...
  if (requestedProviders && requestedProviders.length > 0) {
    ...
    if (providers.length === 0) {
      return { ok: false, error: "Requested providers not available", status: 400, detail: { available, requested: requestedProviders } };
    }
  } else {
  ...
  if (allCandidates.length === 0) {
    ...
    return { ok: false, error: reason, status: 502, detail: {
...
export function getGenerateCandidatesMetadataService(): Result<Record<string, unknown>> {
  const available = getAvailableProviders();
  return ok({
