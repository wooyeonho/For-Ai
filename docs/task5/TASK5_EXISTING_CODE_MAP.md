...
- **Addendum (found during Task 5-B1, not caught by this document's original §1 grep since none of its search terms match the file's content):** `app/api/admin/check-source/route.ts` also calls `fetch(parsed.toString(), { redirect: "follow", ... })` directly, against an admin-operator-typed URL (`requireAdmin`-gated), with only a protocol check and no SSRF protection (no private-IP block, no redirect cap; size is bounded only after the fact via manual byte counting against `MAX_BYTES`).
...
**Classification: `HARDEN` (deferred to Task 5-B1/5-E, not touched in Task 5-0).** `check-source-health.mjs`'s threat model (re-checking known, admin-curated URLs) — and `app/api/admin/check-source/route.ts`'s (a trusted admin operator typing in a URL they already intend to check) — are both materially different from Task 5-B1's `safeFetchExternalSource` threat model (fetching URLs an AI/search step discovered at runtime, i.e. untrusted input). Task 5-B1 must NOT reuse either fetch call as-is; both are explicitly allowlisted in the `external-fetch` CI guard it adds, with this same justification (see `docs/task5/TASK5_B1_EVIDENCE.md` §5). Whether to eventually route either through `safeFetchExternalSource` too is a Task 5-E decision (source-check unification), out of scope here.
