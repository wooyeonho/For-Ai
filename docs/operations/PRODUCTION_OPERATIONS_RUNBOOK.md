...
| Evidence freshness (Task 5-E) | `npm run job:check-evidence-freshness -- --limit=50 --timeout-ms=8000` | Daily | `claim_evidence` (`last_checked_at`/`last_attempt_at`/`consecutive_failure_count`), `evidence_health_checks` (append-only history), `evidence_recheck_cards` (operator queue), `admin_audit_events` action `cron.check_evidence_freshness` | Audit metadata includes per-result counts, cards opened, and skipped storage-backed rows | Scheduler failure alert. **Must be run via the npm script, not the bare `.mjs` file** — the npm script compiles `lib/safe-fetch-external-source.ts` into `.tmp/jobs` first so the job reuses the same SSRF-safe fetch layer as Task 5-B1 instead of an unguarded fetch; running the file directly fails fast with a message pointing back at the npm script. This job never changes `claims.status`, `claims.publication_state`, or citation status — it only logs history and opens operator recheck cards for a human to act on. |
...
| Evidence freshness | Command, timestamp, exit code, `cron.check_evidence_freshness` audit row ID, `checked`, `resultCounts`, `cardsOpened`, and `skippedStorageBacked`. |
