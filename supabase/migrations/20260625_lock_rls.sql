-- Lock down anonymous write access and review-queue reads.
--
...
-- document's view/citation counters). Some schema snapshots also exposed
-- `topic_candidates` SELECT to anon, which leaks the admin review queue.
-- All privileged review reads/writes now flow through
...
-- Review queue SELECT/UPDATE is admin/service-role only.
